...
# HELPERS
# ============================================================
def shannon_entropy(counter_dict: Dict[Any, int]) -> float:
...
def packet_callback(packet):
    ...
    if last_packet_time is not None:
        inter_arrival_times.append(max(0.0, current_time - last_packet_time))
    ...
    if TCP in packet:
        ...
        if (tcp_flags & 0x02) and not (tcp_flags & 0x10):  # SYN without ACK
            syn_packets += 1
        port_packet_count[("tcp", int(packet[TCP].dport))] += 1

    elif UDP in packet:
        udp_packets += 1
        port_packet_count[("udp", int(packet[UDP].dport))] += 1
...
# FEATURE CALCULATION
# ============================================================
def calculate_features() -> Dict[str, Any]:
    duration = time.time() - start_time
    duration = duration if duration > 0 else 1e-9

    packet_rate = packet_count / duration
    syn_ratio = syn_packets / packet_count if packet_count else 0
    udp_ratio = udp_packets / packet_count if packet_count else 0
    tcp_ratio = tcp_packets / packet_count if packet_count else 0
    icmp_ratio = icmp_packets / packet_count if packet_count else 0
    other_ratio = other_packets / packet_count if packet_count else 0

    unique_ip_count = len(unique_ips)
    avg_packets_per_ip = packet_count / unique_ip_count if unique_ip_count else 0
    ...
    if ip_packet_count:
        ...
        top_ip_share = top_count / packet_count if packet_count else 0
    ...
    avg_bytes_per_packet = total_bytes / packet_count if packet_count else 0
...
# MAIN
# ============================================================
def start_feature_extraction(duration: int = 10, iface=None, bpf_filter=None, verbose: bool = True):
    reset_counters()
